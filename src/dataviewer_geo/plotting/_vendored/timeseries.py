...
class Timeseries:
    ...
    def get_neighborhood(
        ...
        from .config import MAX_NEIGHBOR_DISTANCE_KM
    ...
    def plot_time_series(
        ...
        # Note: ensure_* functions are from plotting_joseph.data and are optional
        if lookup_tables is None and master_lookup is not None:
            try:
                ...
                # plotting_joseph.data not available; use basic lookup only
                warnings.warn(
                    "plotting_joseph.data not available; neighbor/country lookups disabled"
                )
                lookup_tables = LookupTables()
